function prepro3.5(soundfile,name)
%preprocessing
...
%     ymax = int64(max(positionarray{i}(2,:)));
%     ymin = int64(min(positionarray{i}(2,:)));
%     phoneme{i} = zeros(ymax-ymin+1,xmax{i}-xmin{i}+1);
    
%     for c = 1:size(positionarray,2)
%         phoneme(positionarray(2,c)+1-ymin,positionarray(1,c)+1-xmin)=1;
%     for row =1:size(phoneme{i},1)
%         for column = 1:size(phoneme{i},2)
%             phoneme{i}(row,column) = clipped(ymin+row,xmin+column);
%         end
%     
%     
end
...
%     for c = 1:size(positionarray{i},2)
%          phoneme(positionarray{i}(2,c)+1-ymin,positionarray{i}(1,c)+1-xmin)=1;
...
% the code above makes a cell where first column labels the start of the
% sound, the second column labling the end of the sound, and the third
% column displaying what the sound is.
% the code below saves the cell into a .txt file. All is great except the
% first row and numbers displayed with too many digits with e and stuff...
% this produces only one .txt file so if ou want to make another one I
% suggest adding a column to the cell for all the labling then modify the code below.
